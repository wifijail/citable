...
function ScanProgress() {
  ...
  useEffect(() => {
    const timer = setInterval(() => setStage((current) => Math.min(current + 1, t.scanner.stages.length - 1)), 1000);
  ...
  return (
    <div className="card flex flex-col items-center gap-6 p-8 sm:flex-row sm:p-10">
      ...
      <ol className="w-full space-y-2.5">
        {t.scanner.stages.map((label, index) => (
          <li
            key={label}
            className={cn(
          ...
          >
            <span
              className={cn(
                ...
                index < stage ? 'border-pass/40 bg-pass/10' : index === stage ? 'border-accent' : 'border-line',
              )}
            >
              {index < stage ? (
...
export function ScanResults() {
  const { busy, error, report, resultsRef } = useScan();
